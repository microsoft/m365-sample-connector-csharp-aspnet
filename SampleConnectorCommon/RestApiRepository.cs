...
namespace Sample.Connector
{
    ...
    public class RestApiRepository : IRestApiRepository
    {
        ...
        public async Task<R> GetRequestAsync<R>(string requestUri, Dictionary<string, string> request, CancellationToken cancellationToken)
        {
            var response = await GetRequestAsync(requestUri, request, cancellationToken);
            return await GetResultFromResponse<R>(response);
        }

        private async Task<HttpResponseMessage> GetRequestAsync(string resource, Dictionary<string,string> queryParams, CancellationToken cancellationToken)
        {
            Uri requestUri = CreateRequestUri(resource, queryParams);
            var response = await this.HttpClient.GetAsync(requestUri.ToString(), cancellationToken);
            return response;
        }

        public async Task<R> PostRequestAsync<T, R>(string requestUri, T request, CancellationToken cancellationToken)
        {
            var response = await PostRequestAsync<T>(requestUri, request, cancellationToken);
            return await GetResultFromResponse<R>(response);
        }

        private async Task<HttpResponseMessage> PostRequestAsync<T>(string requestUri, T request, CancellationToken cancellationToken)
        {
            HttpResponseMessage response = await this.HttpClient.PostAsJsonAsync(requestUri, request, cancellationToken);
            return response;
        }
